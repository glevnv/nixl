...
public:
  ...
  nixl_status_t InitAndRegisterMemory(Blob& blob, nixl_reg_dlist_t& reg_dlist,
                                      nixl_opt_args_t& extra_params, nixlBackendH* backend) {
...
TEST_P(SingleAgentWithMemParamFixture, RegisterMemoryTest) {
  ...
  EXPECT_EQ(agent_helper_->InitAndRegisterMemory(blob, reg_dlist, extra_params, backend), NIXL_SUCCESS);
...
TEST_F(DualAgentBridgeFixture, LoadRemoteMetadataTest) {
  ...
  EXPECT_EQ(local_agent_helper_->CreateBackendWithGMock(local_params, local_backend), NIXL_SUCCESS);
  EXPECT_EQ(remote_agent_helper_->CreateBackendWithGMock(remote_params, remote_backend), NIXL_SUCCESS);

  std::string remote_agent_name_out;
  EXPECT_EQ(local_agent_helper_->GetAndLoadRemoteMD(remote_agent_, remote_agent_name_out), NIXL_SUCCESS);
...
TEST_F(DualAgentBridgeFixture, XferReqTest) {
  ...
  EXPECT_CALL(remote_agent_helper_->GetGMockEngine(), getNotifs).WillOnce([=](notif_list_t &notif_list) {
  ...
  EXPECT_EQ(local_agent_helper_->InitAndRegisterMemory(local_blob, local_reg_dlist,
            local_extra_params, local_backend), NIXL_SUCCESS);
  EXPECT_EQ(remote_agent_helper_->InitAndRegisterMemory(remote_blob, remote_reg_dlist,
            remote_extra_params, remote_backend), NIXL_SUCCESS);
  ...
  EXPECT_EQ(local_agent_->createXferReq(NIXL_WRITE, local_xfer_dlist, remote_xfer_dlist,
            remote_agent_name_out, xfer_req, &local_extra_params), NIXL_SUCCESS);
...
TEST_F(DualAgentBridgeFixture, XferReqSubFunctionsTest) {
  ...
  EXPECT_EQ(local_agent_->prepXferDlist(NIXL_INIT_AGENT, local_xfer_dlist, desc_hndl1), NIXL_SUCCESS);
  EXPECT_EQ(local_agent_->prepXferDlist(remote_agent_name_out, remote_xfer_dlist, desc_hndl2), NIXL_SUCCESS);
  ...
  local_extra_params.notifMsg = msg;
  local_extra_params.hasNotif = true;
  EXPECT_EQ(local_agent_->makeXferReq(NIXL_WRITE, desc_hndl1, indices, desc_hndl2, indices,
            xfer_req, &local_extra_params), NIXL_SUCCESS);
  EXPECT_EQ(local_agent_->postXferReq(xfer_req), NIXL_SUCCESS);
...
TEST_F(DualAgentBridgeFixture, MakeConnectionTest) {
  ...
  EXPECT_EQ(local_agent_helper_->GetAndLoadRemoteMD(remote_agent_, remote_agent_name_out), NIXL_SUCCESS);
  EXPECT_EQ(remote_agent_helper_->GetAndLoadRemoteMD(local_agent_, local_agent_name_out), NIXL_SUCCESS);
